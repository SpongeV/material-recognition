function [T0, T1_idx, T2_idx, T3_idx, T4_idx, Test_idx] = pseudoRandomSubSampling(directory)
...
%			length(clean_data)
	...
	[T1_idx Test_idx] = SplitSetUniformDistributed(T0);
	
	for i=1:dir_count
		% get elements from each directory
% 		temp = T1((i-1)*20+1:i*20);
		tempT2_idx = randperm(20);
		tempT3_idx = randperm(20);
		tempT4_idx = randperm(20);
% 		
% 		T2((i-1)*10+1:i*10) = temp(T2_idx(1:10));
% 		T3((i-1)*4+1:i*4) = temp(T3_idx(1:4));
% 		T4((i-1)*3+1:i*3) = temp(T4_idx(1:3));
		
		T2_idx((i-1)*10+1:i*10) = (i-1)*20 + tempT2_idx(1:10); 
		T3_idx((i-1)*4+1:i*4)   = (i-1)*20 + tempT3_idx(1:4); 
		T4_idx((i-1)*3+1:i*3)   = (i-1)*20 + tempT4_idx(1:3); 
	end
