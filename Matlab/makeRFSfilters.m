...
% Fourier transform.
  ...
  NROTINV=2;					% nr. of rotational invariant filters
  NBAR=length(SCALEX)*NORIENT;	% nr. of bar detection filters
  NEDGE=length(SCALEX)*NORIENT;	% nr. of edge detection filters
  NF=NBAR+NEDGE+NROTINV;		% nr. of total filters
  F=zeros(SUP,SUP,NF);			% allocate matrix for all filters
  hsup=(SUP-1)/2;				% half of the size of the largest filter
